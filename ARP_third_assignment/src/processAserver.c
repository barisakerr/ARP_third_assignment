...
int main(int argc, char *argv[]){
    if((FDlog=open("log/processAserver.log",O_WRONLY|O_CREAT|O_APPEND, 0666))==-1){
        perror("Error opening process A Server log file.");
        exit(1);
    }

    WriteLog("Process A Server started.");
    ...
    //Create socket struct for address and port
    ...
    if(fdServer==-1){
        ...
        WriteLog("Error creating the socket\n");
    }
    if(bind(fdServer, (struct sockaddr *)&address, sizeof(address))==-1){    
        perror("Error binding the socket\n");
        WriteLog("Error binding the socket\n");
        close(fdServer);
    }

    //Listen for incoming connection
    if(listen(fdServer, 1)==-1){
        perror("Error in incoming connection\n");
        WriteLog("Error in incoming connection\n");
        close(fdServer);
    }

    //Accept incoming connection
    int addressLenght=sizeof(address);
    int ClientSocket=accept(fdServer, (struct sockaddr *)&address, (socklen_t *)&addressLenght);
    if(ClientSocket==-1){
        perror("Error accepting incoming connection\n");
        WriteLog("Error accepting incoming connection\n");
        close(fdServer);
    }
    ...
    while(TRUE){
        ...
        } else if(read(ClientSocket, &cmd, sizeof(cmd))>0){ //If it receives a command by a client, the circle moves...

            sem_wait(semaphore0);

            WriteLog("Command received. Moving circle...");
